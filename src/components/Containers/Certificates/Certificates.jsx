...
const Certificates = () => {
  ...
  const removeItem = (certificateOption) => () => {
    if (addedCertificate.length == 5) {
      setAddedCertificate((addedCertificate) =>
        addedCertificate.filter((i) => i !== certificateOption)
      );
      isError();
    } else {
      setAddedCertificate((addedCertificate) =>
  ...
  return (
    <CertificatesForm onSubmit={handleSubmit(onSubmit)}>
      ...
      <SplitButtonDiv
        className={`btnActive ${btnActive ? "active" : "inactive"}`}
      >
        <SplitButton
          id="list"
          list={addedCertificate.map((certificate) => (
            <>
              <LiStyled key={certificate + 1}>{certificate}</LiStyled>
              <Button
                type="button"
                id="Remove"
                title="...&emsp;X"
      ...
      <MoreBtn>
        ...
        {errors.certificates && (
          <ErrorMessage>Empty certificate is not allowed.</ErrorMessage>
        )}
        <ErrorMessage className={`limit ${isActive ? "active" : "inactive"}`}>
          Sorry, only 5 certificates are allowed. You can remove one certificate
          instead.
        </ErrorMessage>
      </MoreBtn>
      <DataInputs>
      ...
      <SubmitButton>
        <Button type="submit" id="Ending" title="Finish" />
      </SubmitButton>
    </CertificatesForm>
  );
};
